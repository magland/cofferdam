...
test('setRepoPrivate and collaborators round-trip through mochi.json', () => {
  const root = makeVaultDir();
  const dir = makeBareRepo(root, 'demo', 'webapp');
  ...
  const raw = JSON.parse(fs.readFileSync(path.join(dir, 'mochi.json'), 'utf8'));
...
test('an unreadable access file fails closed: private, nobody in', () => {
  const root = makeVaultDir();
  const dir = makeBareRepo(root, 'demo', 'webapp');
  fs.writeFileSync(path.join(dir, 'mochi.json'), '{broken');
...
test('a malformed role in the file is dropped rather than trusted', () => {
  const root = makeVaultDir();
  const dir = makeBareRepo(root, 'demo', 'webapp');
  fs.writeFileSync(path.join(dir, 'mochi.json'), JSON.stringify({ collaborators: { bob: 'owner' } }));
