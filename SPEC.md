...
| File | Contents |
|---|---|
| `src/index.ts` | CLI: `serve`, `import`, `collection …`, `user …`, `runner …`, `whoami`, `login`/`logout` |
| `src/cli-api.ts` | The CLI's client for the JSON API: the logged-in target, and calls that report and exit |
| `src/import-cli.ts` | `cofferdam import`, `cofferdam collection add`, `cofferdam collection list`: the client-side import (3.10) |
| `src/source.ts` | What an import may read from: URL, GitHub shorthand, or a local directory |
...
| `src/webops.ts` | UI operations: login/logout, new repo, new collection, file edit/create/delete, branch and tag ops, settings, user admin |
...
| `GET /api/collections`, `GET /api/collections/:name`, `POST /api/collections` | The vault's collections, one collection's repositories, and creating an empty collection `{name}` |
...
| `GET /import` (optional `?collection=`) | Writes out the commands that import an existing repository (section 3.10); performs nothing | session, push scope in the collection |
| `GET/POST /new/collection` | Create a collection with nothing in it | push scope over something in it |
...
- Creating a repository in a new collection creates the collection directory, and so does a push to a new path. The "initialize with a README" option makes the first commit on `main` through the ops layer, and the same create-file flow offers to make the first commit on an empty repository.
- The separate collection-creation flow (`GET/POST /new/collection`, `POST /api/collections`, `cofferdam collection add`) exists for the other order only, where an empty collection is made first and filled by an import or a push afterwards. An empty collection is an empty directory, which `listCollections` already reports with a count of zero, so nothing else had to learn about it. Creating one is authorized by `canCreateCollection` rather than `canPush`, since `canPush` needs a repository name: it matches the collection part of each of the user's push globs, which means a scope of `mycollection/onerepo` may create `mycollection` (its holder can create it by pushing that one repository anyway) and a glob with no slash in it (`*`) covers every collection. There is deliberately no delete: removing an empty collection is `rmdir` in the vault, and a collection with anything in it is deleted one repository at a time.
...
Importing is a client-side operation, and `cofferdam import <source> <collection>[/<name>]` (`src/import-cli.ts`) is what performs it: `git clone --bare` the source into a directory under the system temporary directory, `git push --mirror` at a vault URL that does not exist yet, remove the clone. Push-to-create makes the repository through `ops.createRepo`, so it arrives with the same `receive.*` protections as any other, HEAD pointed at the right branch, and branches and tags in place. `--lfs` adds `git lfs fetch --all` before the push and `git lfs push --all` after it.
...
Three details of the command are load-bearing. The push is authorized by a credential helper written on the fly, `-c credential.helper=` to clear the list and then one that prints `$COFFERDAM_USER` and `$COFFERDAM_TOKEN`, so the token reaches git through the environment rather than through a command line every process on the machine can read, and no prompt can appear (`GIT_TERMINAL_PROMPT=0` makes that a failure rather than a wait). The clone runs with the environment untouched, so a private source is read by whatever credentials the operator already has for it. And failures inside the clone-and-push are raised as exceptions rather than `process.exit`, because exiting skips the `finally` that removes the temporary clone.

Before cloning, the command asks `/api/whoami` who the token belongs to and `/api/collections/:name` what is already in the collection, so a bad token or a name already taken stops it in a second rather than after a clone that may take minutes. A mirror push at an existing repository would replace its refs, which is why an existing name is refused rather than merged into.

`GET /import` performs nothing at all: it writes out the commands, with the collection and the vault's own URL filled in, and refuses a collection the viewer has no scope in, since handing someone a command their token cannot run is worse than a refusal. Earlier this page asked for the source in a form and answered with a shell one-liner, which put a form in front of an operation the page could not perform and made the source a string headed for somebody's shell; now the CLI performs it and the page only has to say what to run. The git commands remain below the `cofferdam import` one, with the source and the name as placeholders, for a machine with no Node on it, which is why `parseSource` keeps its allowlist of harmless characters even though nothing pastes a source anywhere any more.

We chose all of this over a server-side import deliberately, and the reasoning should be revisited rather than assumed. A server that clones on the operator's behalf needs outbound network access, a host allowlist and address checks against SSRF, somewhere to put credentials for private sources, a disk budget, and work that outlives a request, which is machinery this project has none of. Doing it on the operator's machine needs none of that: their existing git credentials read the source, their cofferdam token writes the vault, and progress and cancellation come from the terminal. The cost is that the data passes through their machine and that bulk import is a shell loop rather than a form. Revisit if a job model arrives for another reason, and note that `import` is a reserved name (`src/scan.ts`) reserved for exactly that possibility.

A source may also be a directory on the machine running the command, which is what a repository that exists only as a local clone needs, and what lets the smoke suite test a real import with no network. A path is recognized before the `owner/repo` shorthand, since the two have the same shape and the directory in front of you is what you meant.
