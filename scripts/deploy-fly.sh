#!/usr/bin/env bash
# Create a remote cofferdam vault on Fly.io, from nothing to a working server.
#
...
for _ in 1 2 3 4 5 6; do
  TOKEN="$(fly logs -a "$APP" --no-tail 2>/dev/null | grep -o 'cofferdam_[0-9a-f]\{64\}' | head -1 || true)"
  [ -n "$TOKEN" ] && break
  sleep 5
done
...
if [ -n "$TOKEN" ]; then
  ...
  echo "  $TOKEN"
  echo ""
  echo "Log in to the vault with it, and later commands need nothing else:"
  echo ""
  echo "  cofferdam login $URL"
  echo "  cofferdam whoami"
else
