...
# machine's entrypoint where there is no command line to edit: MOCHI_HOST
# and MOCHI_RUNNER_TOKEN say which vault and which runner,
# MOCHI_WAKE_SECRET what a wake request must present, and the two below
...
node /app/dist/index.js runner run \
  --host "$MOCHI_HOST" \
  --idle "${MOCHI_IDLE:-5m}" \
  --wake-port "${MOCHI_WAKE_PORT:-3000}" \
  "$@" &
RUNNER_PID=$!
