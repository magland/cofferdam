 ...
 * then MOCHI_REPO, then the git remote here that points at this vault.
...
export async function resolveRepo(
  ...
  const env = process.env.MOCHI_REPO?.trim();
  if (env) return parseRef(env, 'MOCHI_REPO');
  const remote = await fromGitRemotes(target.host);
  if (remote) return remote;
  throw new CliError(
    `Which repository? Name it as <collection>/<repo>, pass --repo <collection>/<repo>, set MOCHI_REPO,\n` +
