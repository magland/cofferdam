...
// program with a context window: `cofferdam --help` lists groups, `cofferdam
// issue --help` lists that group, and `cofferdam issue create --help` lists one
// command's options. `cofferdam commands --json` dumps the whole registry for a
...
export interface Command {
  ...
  path: string[];
  summary: string;
  /** Longer help, shown by `cofferdam <command> --help`. */
  description?: string;
  options?: OptionSpec[];
  args?: ArgSpec[];
  /**
...
function parseOptions(cmd: Command, argv: string[]): { values: Map<string, Value>; args: string[] } {
  ...
  for (let i = 0; i < argv.length; i++) {
      ...
      if (!spec) {
        ...
        usageError(
          `unknown option --${name} for '${cmd.path.join(' ')}'` +
            (near ? `; did you mean --${near}?` : `. Run 'cofferdam ${cmd.path.join(' ')} --help'.`)
        );
      }
    ...
    if (a.startsWith('-') && a.length > 1) {
      ...
      if (!spec) usageError(`unknown option ${a} for '${cmd.path.join(' ')}'. Run 'cofferdam ${cmd.path.join(' ')} --help'.`);
...
function checkArgs(cmd: Command, args: string[]): void {
  ...
  if (args.length < required) {
    usageError(`'cofferdam ${cmd.path.join(' ')}' needs ${usageLine(cmd)}`);
  }
  const variadic = specs.some((s) => s.variadic);
  if (!variadic && args.length > specs.length) {
    usageError(`unexpected argument '${args[specs.length]}' to 'cofferdam ${cmd.path.join(' ')}'`);
  }
}
...
export async function dispatch(cli: Cli, argv: string[]): Promise<void> {
  ...
  if (!found) {
    ...
    throw new CliError(
      `unknown command '${head}'` + (near ? `; did you mean '${near}'?` : ". Run 'cofferdam --help'."),
      EXIT_USAGE
    );
  }
