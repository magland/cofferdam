...
// The bearer-token JSON API used by the repos CLI. Only Bearer tokens are
...
export function registerApi(app: Express, root: string): void {
  ...
  function requireApiAuth(req: Request, res: Response): AuthResult | null {
    ...
    if (!m) {
      apiError(res, 401, 'missing bearer token; set REPOS_TOKEN');
      return null;
    }
  ...
  app.post('/api/users', (req, res) => {
    ...
    if (existing) {
      if (scope || admin) {
        apiError(res, 409, `user ${username} already exists; use 'repos user grant' to extend it`);
        return;
      }
