...
// composite action, or an action hubbit implements itself because the real
...
export interface StepExecContext {
  ...
  // runner sees on the host. Actions that hand hubbit a path (an artifact to
  ...
  return (raw: string) => {
    ...
    switch (cmd.name) {
      ...
      case 'debug':
        ...
        // run was started with debug logging, and so does hubbit.
      ...
      case 'stop-commands':
        break;
      // Problem matchers annotate a run's diff on GitHub. hubbit has no
...
async function runActionStep(
  ...
  if (override) {
    ctx.log(`Using hubbit's built-in ${override.name} (${ref.raw})`);
    try {
