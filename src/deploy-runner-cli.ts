...
// `feorge deploy fly runner`: put a workflow runner on Fly.io, beside a
...
const HOST_SECRET = 'FEORGE_HOST';
const TOKEN_SECRET = 'FEORGE_RUNNER_TOKEN';
const WAKE_SECRET = 'FEORGE_WAKE_SECRET';
...
function flyToml(app: string, s: Settings): string {
  return `# Generated by feorge deploy fly runner for '${app}'. Written to a temporary
...
[env]
  FEORGE_IDLE = "${s.idle}"
  FEORGE_WAKE_PORT = "${INTERNAL_PORT}"
...
function writeTempConfig(app: string, s: Settings): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'feorge-runner-deploy-'));
...
export async function deployFlyRunnerCmd(args: string[], usage: () => never): Promise<void> {
  ...
  if (!registered && a.allow.length === 0) {
    die(
      `No runner named '${name}' is registered with ${target.host}, so this deploy would\n` +
        ...
        `  feorge deploy fly runner ${app} --allow 'mycollection/*'\n`
    );
  }
  ...
  if (registered && a.allow.length > 0 && !sameAllow) {
    die(
      `Runner '${name}' is already registered, serving ${registered.allow.join(', ')}.\n` +
        ...
        `  feorge runner remove ${name}\n` +
        `  feorge deploy fly runner ${app} --allow '<globs>'\n`
    );
  }
  ...
  if (code !== 0) {
    ...
    if (buildRoot === null && a.image === null) {
      ...
      console.error('');
      console.error(`  feorge deploy fly runner ${app} --from-source`);
      console.error('');
  ...
  console.log('');
  console.log('  feorge runner list');
  console.log(`  feorge runner wake ${name}      # start it now, and time how long that takes`);
  console.log(`  fly logs -a ${app}`);
  console.log('');
  console.log(`  feorge deploy fly runner ${app} --vm-size shared-cpu-4x --idle 15m`);
}

export async function deployFlyRunnerShowCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseArgs(args, usage);
  if (!a.app) die('Which app? Usage: feorge deploy fly runner show <app>');
  rejectShapingFlags(a, 'feorge deploy fly runner show <app>', false);
  const app = a.app;
  await requireFly();
...
export async function deployFlyRunnerDestroyCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseArgs(args, usage);
  if (!a.app) die('Which app? Usage: feorge deploy fly runner destroy <app> [--yes]');
  rejectShapingFlags(a, 'feorge deploy fly runner destroy <app> [--yes]', true);
  const app = a.app;
  await requireFly();
  if (!(await appExists(app))) {
    die(`No Fly app named '${app}' that you can see. Check the name, or: fly apps list`);
  ...
  if (orphan && target) {
    ...
    console.log('');
    console.log('  feorge runner list');
    console.log('  feorge runner remove <name>');
  }
}
