...
// `cofferdam deploy fly runner`: put a workflow runner on Fly.io, beside a
...
const HOST_SECRET = 'COFFERDAM_HOST';
const TOKEN_SECRET = 'COFFERDAM_RUNNER_TOKEN';
const WAKE_SECRET = 'COFFERDAM_WAKE_SECRET';
...
function flyToml(app: string, s: Settings): string {
  return `# Generated by cofferdam deploy fly runner for '${app}'. Written to a temporary
...
[env]
  COFFERDAM_IDLE = "${s.idle}"
  COFFERDAM_WAKE_PORT = "${INTERNAL_PORT}"
...
function writeTempConfig(app: string, s: Settings): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cofferdam-runner-deploy-'));
...
export async function deployFlyRunnerCmd(args: string[], usage: () => never): Promise<void> {
  ...
  if (registered && a.allow.length > 0 && !sameAllow) {
    die(
      `Runner '${name}' is already registered, serving ${registered.allow.join(', ')}.\n` +
        ...
        `  cofferdam deploy fly runner ${app} --allow '<globs>'\n`
    );
  }
  ...
  console.log('');
  console.log('  cofferdam runner list');
  console.log(`  cofferdam runner wake ${name}      # start it now, and time how long that takes`);
  console.log(`  fly logs -a ${app}`);
  console.log('');
  console.log(`  cofferdam deploy fly runner ${app} --vm-size shared-cpu-4x --idle 15m`);
}

export async function deployFlyRunnerShowCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseArgs(args, usage);
  if (!a.app) die('Which app? Usage: cofferdam deploy fly runner show <app>');
  rejectShapingFlags(a, 'cofferdam deploy fly runner show <app>', false);
  const app = a.app;
  await requireFly();
...
export async function deployFlyRunnerDestroyCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseArgs(args, usage);
  if (!a.app) die('Which app? Usage: cofferdam deploy fly runner destroy <app> [--yes]');
  rejectShapingFlags(a, 'cofferdam deploy fly runner destroy <app> [--yes]', true);
  const app = a.app;
  await requireFly();
  if (!(await appExists(app))) {
    die(`No Fly app named '${app}' that you can see. Check the name, or: fly apps list`);
  ...
  if (orphan && target) {
    ...
    console.log('');
    console.log('  cofferdam runner list');
    console.log('  cofferdam runner remove <name>');
  }
}
