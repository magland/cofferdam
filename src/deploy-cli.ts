...
// `feorge deploy fly`: put a vault on Fly.io from one command, and deploy
...
export const IMAGE_REPO = 'ghcr.io/magland/feorge';
const VOLUME_NAME = 'vault';
const OWNER_TOKEN_SECRET = 'FEORGE_OWNER_TOKEN';
 ...
 * installed feorge has no Dockerfile and no src to build from, and that is
...
export function sourceRoot(): string {
  ...
  if (!fs.existsSync(path.join(root, 'Dockerfile')) || !fs.existsSync(path.join(root, 'src'))) {
    die(
      '--from-source builds the image from a feorge checkout, and this is not one:\n' +
        `  ${root}\n\n` +
        ...
        '  git clone https://github.com/magland/feorge && cd feorge && npm install\n' +
...
function flyToml(app: string, s: Settings): string {
  return `# Generated by feorge deploy for '${app}'. Written to a temporary
...
function writeTempConfig(app: string, s: Settings): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'feorge-deploy-'));
...
export async function deployFlyCmd(args: string[], usage: () => never): Promise<void> {
    ...
    process.on('exit', () => {
      ...
      fs.writeSync(
        ...
        '\nThe owner token this run staged as a Fly secret, shown here because nothing\n' +
          ...
          `\n  ${ownerToken}\n` +
          `\nKeep it, then: feorge login ${appUrl(app)} --token ${ownerToken}\n`
      );
    });
  }
  ...
  if (!ready.ok) {
    ...
    if (existed) {
      ...
      console.error('');
      console.error(`  feorge login ${url}`);
    }
  ...
  // it, and `feorge login` stays the one thing that stores a credential.
  console.log('');
  ...
  console.log('');
  console.log('  feorge whoami');
  console.log("  feorge user add alice --scope 'alice/*'");
  console.log(`  feorge import https://github.com/someone/something.git mine`);
  console.log('');
  console.log('Deploy an update, or change a setting, with the same command:');
  console.log('');
  console.log(`  feorge deploy fly ${app}`);
  console.log(`  feorge deploy fly ${app} --volume 50 --vm-memory 1gb`);
}

export async function deployShowCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseDeployArgs(args, usage);
  if (!a.app) die('Which app? Usage: feorge deploy fly show <app>');
  rejectFlyFlags(a, 'feorge deploy fly show <app>', false);
  const app = a.app;
  await requireFly();
  ...
  if (saved && saved.host.replace(/\/+$/, '') === url) console.log('  login     this is the vault feorge commands use');
  ...
  console.log(backup ? `  backup    ${backup}` : '  backup    none on this machine (feorge backup <dir>)');
  console.log('');
  console.log(`  fly logs -a ${app}`);
}
...
export async function deployDestroyCmd(args: string[], usage: () => never): Promise<void> {
  const a = parseDeployArgs(args, usage);
  if (!a.app) die('Which app? Usage: feorge deploy fly destroy <app> [--yes]');
  rejectFlyFlags(a, 'feorge deploy fly destroy <app> [--yes]', true);
  const app = a.app;
  await requireFly();
  if (!(await appExists(app))) {
    die(`No Fly app named '${app}' that you can see. Check the name, or: fly apps list`);
