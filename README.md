...
git asks for a password on the push: that is your doqpod token. The push creates the repository, so the target must not exist yet, and your push scope has to cover it. Branches and tags come across. Issues, pull requests, and git-lfs objects do not, and the description is set afterwards in repository settings.
