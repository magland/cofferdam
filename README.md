...
- **Browsing:** files at any ref, syntax highlighting, markdown with KaTeX, history, diffs, blame, compare, search, contributors, archives. Anonymous, apart from private repositories. A collection introduces itself with a profile README, from a `.feorge` repository in it.
...
- **CLI and JSON API** covering everything the web UI does, plus a generic `feorge api`. Built for scripts: `--json` everywhere, distinct exit codes, no prompts.
...
No database, no state outside the directory. Backup is `cp -a`, migration is `rsync`, and `feorge backup <dir>` pulls the same copy over HTTP where you have no shell. The server reads disk on every request, so a running vault can be read and grepped with ordinary tools.

## Documentation

- [Getting started](docs/getting-started.md) - a local vault, a public one, and a domain of your own
- [The command line](docs/cli.md) - the `feorge` command and its subcommands
...
- [feorge for an agent](docs/agents.md) - short enough to paste into a context window

## Using a vault from Claude Code

[feorge-skill](https://github.com/magland/feorge-skill) teaches an agent this CLI the way it already knows `gh`.

```
/plugin marketplace add magland/feorge-skill
/plugin install feorge@feorge-skill
```

The agent needs `feorge` on its PATH and either `FEORGE_HOST`/`FEORGE_TOKEN` or a completed `feorge login`.
...
The example vault has site admin `dev` with token `feorge_example_dev_token` (example vault only) and plain user `reader` with `feorge_example_reader_token`.
