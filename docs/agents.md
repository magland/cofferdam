...
`cofferdam commands --json` dumps the whole command set as data, which is more reliable than this list.
...
```bash
cofferdam api repos/demo/proj/commits -X POST --field branch=main \
...
```bash
cofferdam branch create fix-thing
cofferdam file write src/thing.ts --branch fix-thing --body-file patched.ts
cofferdam pr create --base main --head fix-thing --title 'Fix the thing'
cofferdam api repos/demo/proj/pulls/7/merge     # would it apply? ask before merging
cofferdam pr merge 7 --squash --delete-branch
```
...
```bash
cofferdam workflow run .github/workflows/test.yml --field target=all
cofferdam run watch 42 --exit-status
cofferdam run view 42 --log          # the failed job's log, last 200 lines
