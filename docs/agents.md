...
Short enough to paste into a context window. If you know `gh`, most of this will be guesses you would have made anyway.
...
`feorge commands --json` dumps the whole command set as data, which is more reliable than this list.
...
```bash
feorge api repos/demo/proj/commits -X POST --field branch=main \
...
```bash
feorge branch create fix-thing
feorge file write src/thing.ts --branch fix-thing --body-file patched.ts
feorge pr create --base main --head fix-thing --title 'Fix the thing'
feorge api repos/demo/proj/pulls/7/merge     # would it apply? ask before merging
feorge pr merge 7 --squash --delete-branch
```
...
- **No `feorge restore`.** `feorge backup <dir>` makes a copy whose `<dir>/current` is itself a vault, so restoring is `feorge serve <dir>/current` or copying that directory onto a host. Nothing reconciles a backup against a running vault.
