...
```bash
feorge runner add laptop --allow 'mycollection/*'    # on any machine, with an admin token
feorge runner run --host https://vault.example.com --runner-token feorge_runner_...
```
...
The expression functions are `contains`, `startsWith`, `endsWith`, `format`, `join`, `toJSON`, `fromJSON`, `success`, `failure`, `always`, and `cancelled`. `hashFiles` is the one GitHub has that feorge does not, since it exists to key a cache that is also not here.
...
A few actions are not ordinary programs: they are clients for services that exist only inside GitHub. Running them verbatim against a vault cannot work, so feorge substitutes its own implementation of the same interface, chosen by the `uses:` string and applied at any nesting depth, including inside somebody else's composite action.

| `uses:` | What feorge does instead |
| --- | --- |
...
| `actions/configure-pages` | Reports this vault's site URL and base path, and exports `FEORGE_SITE_BASE_PATH` |
| `actions/deploy-pages` | Publishes the `github-pages` artifact as the repository's site |

Everything else runs unmodified, `actions/setup-node` and the rest included. Note that `actions/upload-pages-artifact` is *not* substituted: it is an ordinary composite action that tars a directory and calls `upload-artifact`, so the real one works as it is, on top of feorge's `upload-artifact`.
...
feorge checks the repository out into the workspace before the job starts. On GitHub the workspace begins empty and `actions/checkout` fills it, and feorge's `checkout` is a re-sync of what is already there. A workflow that deliberately wants an empty workspace will be surprised.
...
```bash
feorge runner add laptop --allow 'mycollection/*' --labels ubuntu-latest
```
...
Job workspaces are made under the system temporary directory, which `--work-dir` changes, and the container joins Docker's default network unless `--network` names another. `FEORGE_RUNNER_TOKEN` supplies the runner's token where a command line is the wrong place for it, as in a systemd unit or a container.
...
```bash
feorge runner run --idle 5m --wake-port 3000
```
...
```bash
feorge runner wake myrunner --wake-url https://my-runner.fly.dev/wake
```
...
Two things follow from this arrangement that are worth saying plainly. A runner in the stopped state is not a fault: it is the arrangement working, and `feorge runner list` says "woken on demand" rather than reporting it as absent. And the first job after a stop waits for a boot, which on a Fly machine with a Docker daemon to start is around half a minute; jobs that follow it do not. `feorge runner wake <name>` sends the request by hand and reports how long the runner took to answer, which is the way to test an address without queuing a job.
...
`feorge deploy fly runner` puts all of the above on Fly in one command, beside a vault that is already there or anywhere else:
...
A run that sits at `queued` has two usual causes, and `feorge runner list` reports both:
