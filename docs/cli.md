...
The `feorge` command, how it is configured, how pushing is authorized, and the API underneath it.
...
```bash
feorge login http://127.0.0.1:3000    # asks for the token, without echo
feorge whoami
feorge user list
feorge collection add mycollection
feorge import https://github.com/owner/repo mycollection
```

`feorge login` is the way to configure the CLI for a person at a keyboard: no token to re-supply per command, and nothing to set in the environment. The vault URL is remembered in `~/.config/feorge/login.json` (mode 0600) and the token goes to git's own credential store, which is where git needs it anyway for pushing, so a token is kept in one place rather than two (see [Not typing the token every time](#not-typing-the-token-every-time)). `feorge logout` undoes both.

A caller in a container is in a different position: it has no keyring, may have no writable home directory, and gets its secrets as environment variables. So `FEORGE_HOST` and `FEORGE_TOKEN` are honoured by every command that talks to a vault, and `--token-stdin` reads a token from stdin so that it appears in neither argv nor shell history. Precedence for both the host and the token is the same: the option, then the environment, then what `feorge login` left behind.

```bash
export FEORGE_HOST=https://vault.example.com
export FEORGE_TOKEN="$(cat /run/secrets/feorge)"
feorge whoami --json
```

(`feorge runner run` reads `FEORGE_RUNNER_TOKEN` as well, since a runner holds a token that is not any user's.)

`feorge deploy fly <app>` is the exception to the division above in one respect: it drives flyctl rather than a vault, since at the moment it runs there is no vault yet. It creates a vault on Fly.io, or deploys an update to an existing one, and on a new one it mints the owner token locally and prints it once the server has confirmed it, together with how to sign in on the web and the `feorge login` line that stores it for the CLI and git (see [Deploying a vault](deploying.md)). The deploy itself stores nothing: logging in stays a separate, deliberate step.

`feorge deploy fly runner <app>` is the one command that drives both at once: it registers a runner with the vault you are logged in to, and then creates the Fly app that will run it. The runner it deploys stops when it has been idle, and the vault starts it again when a job is waiting, which is the arrangement described in [Workflows](workflows.md#a-runner-that-stops-when-it-is-idle).

Note that login is a client-side arrangement only: it calls the server once to check who the token belongs to, and writes nothing but local files. `feorge runner run` is the one command that reads a configuration of its own, since a runner holds a token that is not any user's: it is a long-running process that takes workflow jobs from a vault and executes them locally in Docker (see [Workflows](workflows.md)).
...
```bash
feorge --help              # command groups, and the commands that stand alone
feorge user --help         # the commands in one group
feorge user add --help     # one command's arguments and options
feorge commands --json     # every command, argument, and option, as data
```

`feorge commands --json` is the whole registry, which is enough to discover the surface without reading any of this.

### Output and exit codes

Every command that reads something takes `--json`, which puts a single JSON value on stdout and sends every diagnostic to stderr, so `feorge issue list --json | <parser>` never has to filter anything out. A comma-separated field list keeps only those fields, as `gh` does: write it attached, `--json=number,title`, or detached when it names more than one field, `--json number,title`. A name that is not a field of the response is an error naming the ones that are.
...
```bash
feorge repo list                        # every repository in the vault
feorge repo view demo/proj
feorge branch list --repo demo/proj
feorge file list --repo demo/proj       # one directory
feorge file list --all                  # every path in the tree
feorge file list --commits              # with the last commit per entry
feorge file view README.md
feorge file view logo.png --raw         # the bytes, unchanged
feorge commit list --limit 10
feorge commit list --path src/index.ts  # only commits touching a path
feorge commit view <sha> --patch
feorge diff main...topic
feorge diff main...topic --stat         # names and counts, not the patch
feorge search 'needle'
```
...
Which repository a command is about is resolved in this order: the positional argument or `--repo <collection>/<repo>`, then `FEORGE_REPO`, then the git remote in the current directory that points at the vault you are logged in to, preferring `origin`. A remote for some other host is not an answer, so a clone of a GitHub repository is never read as naming something here. Failing all three, the command says so and names all three.

### Changing a repository

```bash
feorge repo create mycollection/thing --description 'A thing' --readme
feorge repo edit --description 'A better thing'
feorge repo fork demo/proj myfork
feorge repo rename demo/proj newname --collection othercollection
feorge repo delete demo/old --yes
feorge repo clone demo/proj

feorge branch create topic              # from the default branch, or from a named one
feorge branch create topic main
feorge branch delete topic --yes
feorge tag list
feorge tag create v1.0.0 main
feorge tag delete v1.0.0 --yes

feorge file write notes.md --message 'Add notes' < notes.md
feorge file delete notes.md --yes
```
...
`feorge file write` reads the content from `--body`, from `--body-file`, or from stdin when neither is given, so a generated file can be piped straight in. Given `--expected-sha`, a branch that has moved since is a conflict (exit 5) rather than a silent overwrite, which is exactly what a caller that reads, thinks, and then writes wants:

```bash
sha="$(feorge file view config.json --json=commit | jq -r .commit)"
# ... work out the new content ...
feorge file write config.json --expected-sha "$sha" --body-file new.json
```
...
```bash
feorge api repos/demo/proj/commits -X POST --input change.json
```
...
```bash
feorge file write notes.md --new-branch topic --message 'Add notes' --body-file notes.md
feorge pr create --base main --head topic --title 'Add notes'
```

### Issues and pull requests

```bash
feorge issue list --state all
feorge issue view 12 --comments
feorge issue create --title 'It broke' --body-file report.md --label bug
feorge issue edit 12 --add-label urgent
feorge issue comment 12 --body-file -        # from stdin
feorge issue close 12
feorge issue reopen 12

feorge pr list
feorge pr view 4 --comments
feorge pr create --base main --head topic --title 'Add a thing'
feorge pr diff 4
feorge pr comment 4 --body 'Looks right to me'
feorge pr merge 4 --squash --delete-branch
feorge pr checks 4
feorge pr close 4
feorge pr reopen 4
```
...
```bash
feorge api repos/demo/proj/pulls/4/merge
```
...
`feorge pr checks` deserves its own note: feorge has no equivalent of a check suite or a commit status, so there is nothing behind that command but the workflow runs whose commit is the pull request's head. That is what it reports, and that is all it means.

### Workflows and their runs

```bash
feorge workflow list                        # workflows at a ref, and the inputs each takes
feorge workflow run .github/workflows/build.yml --field greeting=hello
feorge run list --status completed
feorge run view 12                          # the run and its jobs
feorge run view 12 --log                    # and the failed job's log
feorge run watch 12 --exit-status
feorge run cancel 12
feorge run rerun 12
feorge run download 12 --dir artifacts
```
...
`feorge run watch` polls until the run finishes and then reports how it went; `--exit-status` makes a failed run a non-zero exit, which is what a script wants. It polls rather than streams because the engine has no event channel and the vault reads its state off disk per request, so a five-second poll against a local process costs nothing and needs no protocol. Note that a vault with no runner registered queues its runs and waits, so a watch there will reach its timeout; `feorge runner list` says whether one is connected and what is queued. A poll that cannot reach the vault at all is not treated as an answer about the run: the watch says so once, keeps polling, and says when the vault answers again, because a restart or a dropped connection is not a reason to abandon a run that is still going. The `--timeout`, which is the caller's actual patience, still ends it, and reports the unreachability if that is what is still true.

### Releases, users, and settings

```bash
feorge release list
feorge release view v1.0.0
feorge release create v1.0.0 --title 'First cut' --notes-file NOTES.md
feorge release edit v1.0.0 --latest          # clear the prerelease flag
feorge release delete v1.0.0 --yes

feorge user view alice
feorge user token list alice
feorge user token revoke alice <token-id> --yes
feorge user delete alice --yes
feorge collection rename mycollection newname
feorge collection delete emptyone --yes

feorge config view
feorge config set --theme slate --ci-runs 50
feorge config set --ci-days 30 --ci-artifact-mb 100
feorge config set --sites-host vault-sites.example.org
feorge config set --egress-gb-per-day 50
```

A release hangs on a tag that already exists: notes for a tag nobody can check out are notes about nothing, so `feorge tag create` comes first. Creating and editing are the same call underneath, since a release is keyed by its tag, which takes a whole class of "already exists, retry as an edit" logic out of a caller. Deleting a release deletes its notes and leaves the tag; the two are separate operations.
...
`feorge collection rename` moves everything the collection holds: the repositories, their issues, pull requests, releases, sites, run histories, and LFS objects. It is one directory rename, so it costs the same on a collection of a hundred gigabytes as on an empty one. There is no `--yes`, since a rename that was a mistake is undone by renaming back; what it takes is ownership of the collection, and the owners travel with it. The old address is redirected to the new one, so links and existing clones keep working until something else is created under that name; see [The old address](vault.md#the-old-address). One thing does not move with it: token scopes naming the old collection cover nothing afterwards and have to be granted again under the new name. `feorge repo rename` is the same operation one level down, and can move a repository to another collection with `--collection`, which additionally takes permission to create in the destination.
...
`feorge config set` reaches the theme, the CI retention settings, the hostname repository sites are served from, and the daily cap on outgoing bytes. Those are the settings the server consults per request, so a change is in effect on the next one; `--sites-host ''` clears it, putting sites back on the vault's own hostname under the sandbox, and a value that is not a plausible hostname is refused rather than stored (see [A domain of your own](deploying.md#a-domain-of-your-own)). This is the reason there is a command at all: a vault's settings should not need a shell on the machine holding its disk.

`--egress-gb-per-day` caps the bytes the vault may send in one UTC day, 20 GB by default and `0` for no cap. Past it, ordinary requests are refused with `503` until the next UTC midnight, while the administration pages and signing in keep working so the cap can be raised from the vault itself. `feorge api /api/egress` shows what has gone out today, per repository, which is the same breakdown `/admin/egress` draws. See [Outgoing bytes](deploying.md#outgoing-bytes).
...
```bash
feorge backup ~/backups/myvault              # incremental sync over HTTP
feorge backup ~/backups/myvault --snapshot   # ...then snapshot, then prune
feorge backup list ~/backups/myvault
feorge backup verify ~/backups/myvault
feorge backup prune ~/backups/myvault
```

A vault is a directory, so a copy of one is a directory too, and `feorge backup` pulls it over HTTP: no shell on the server, no flyctl, no rsync at the far end. The backup directory is itself a vault, so restoring is `feorge serve ~/backups/myvault/current`. Repositories come across as mirrors and a repository nothing was pushed to is skipped without a request; everything else is compared by size and modification time and fetched only where it differs. The token needs to belong to a site admin, since the copy includes `vault.json`.
...
### Reaching any route: `feorge api`

`feorge api` sends a request to any route of the JSON API and prints what comes back, so a capability with no typed command of its own is still one line away:

```bash
feorge api whoami
feorge api collections -X POST --field name=mycollection
feorge api collections/mycollection
```
...
`--include` (`-i`) prints the status and content type as well. Note that it prints them on **stderr**, not stdout, so that `feorge api ... -i | jq` still sees only the body; redirect stderr if you want them together.
...
Because the token lives in git's store rather than in a feorge file, any helper git can use will do, including one of your own that fetches the token from elsewhere:

```bash
git config --global 'credential.https://vault.example.com.helper' \
  '!f(){ echo username=jeremy; echo "password=$(my-secret-tool get feorge)"; }; f'
```

Such a helper stores nothing, so `feorge login` against it does no more than record the vault and confirm that reading the credential back yields the token it just checked, which is all it needs to do. The trade-off is that this works only where whatever the helper calls works, so editors, GUI git clients, and cron jobs may see no credential at all.

### Importing an existing repository

Importing runs on your machine, not on the server, and `feorge import` is what runs it:
...
The source is read with whatever git credentials this machine already has, so a private source works if your own `git clone` of it works, and the push is authorized by the token `feorge login` stored. Branches and tags come across. Issues and pull requests do not. A name already taken stops the import before the clone, since a mirror push would replace that repository's branches and tags.
...
```bash
feorge import https://github.com/owner/repo mycollection --lfs
```
...
```bash
feorge user add alice                          # alice owns the collection 'alice' from the start
feorge collab add mycollection/webapp alice    # the write role on one repository
feorge collab add mycollection/webapp bob --role read   # read: sees it even when private
feorge collab list mycollection/webapp
feorge collection owner add mycollection alice # the admin role on everything in mycollection
feorge user grant alice --site-admin           # everything, everywhere
feorge user list                               # review who holds tokens and the site-admin bit
```

Note that `--site-admin` on `feorge user add` applies only when creating a user; on an existing user the command refuses rather than silently escalating (run it plain to mint an additional token).

### Private repositories

`feorge repo create mycollection/secrets --private` creates a repository only its collaborators, the collection's owners, and site admins can see; `feorge repo edit mycollection/secrets --public` (or `--private`) flips an existing one, taking the admin role. To everyone else a private repository answers the same 404 an absent one does, in listings, over git, and in the API alike.
...
If you are writing something that will drive this rather than reading it yourself, [feorge for an agent](agents.md) is a page and a half, and includes an honest list of what feorge does not have.
