...
Opening a `*.age` file shows a card instead of the file: the file's size, a passphrase field, and a Decrypt button. The ciphertext is fetched from the same raw endpoint the Raw button uses, the passphrase is fed to the decryption in the page, and the plaintext replaces the card, rendered by its inner name: `secrets.md.age` renders as markdown, anything else as plain text. A Lock button puts the card back. The passphrase and the plaintext exist only in the open page; neither is sent anywhere, cached anywhere, or stored anywhere, and navigating away discards both.
